...
static E_api S_Get_enum_api ( char *api ) {
	...
	if ( strcmp ( api , "self_test" ) == 0 ) {
		return E_self_test ;
	} else if ( strcmp ( api , "light" ) == 0 ) {
		return E_light ;
	} else if ( strcmp ( api , "air_conditioner" ) == 0 ) {
		return E_air_conditioner ;
	} else if ( strcmp ( api , "air_purifier" ) == 0 ) {
		return E_air_purifier ;
	} else if ( strcmp ( api , "sweeping_rebot" ) == 0 ) {
		return E_sweeping_robot ;
	}else if ( strcmp ( api , "dehumidifier" ) == 0 ) {
		return E_de_humidifier ;
	}
	return E_null ;
}
	...
	* output_e_api = S_Get_enum_api ( api ) ;
	...
	switch ( * output_e_api ) {
		case E_null : {
			...
			break ;
		case E_self_test : {
			in_request *p_request = ( in_request * ) calloc ( 1 , sizeof(in_request) ) ;
			if ( NULL == p_request ) {
				return NULL ;
			}
			p_request->m_e_api = * output_e_api ;
			rtn = Json_value ( inputData , "api" , p_request->m_api , sizeof ( p_request->m_api ) ) ;
			if ( D_success != rtn ) {
				free ( p_request ) ;
				p_request = NULL ;
				return NULL ;
			}
			rtn = Json_value ( inputData , "api_type" , p_request->m_api_type , sizeof ( p_request->m_api_type ) ) ;
			if ( D_success != rtn ) {
				free ( p_request ) ;
				p_request = NULL ;
				return NULL ;
			}
			rtn = Json_value ( inputData , "req_md5" , p_request->m_req_md5 , sizeof ( p_request->m_req_md5 ) ) ;
			if ( D_success != rtn ) {
				free ( p_request ) ;
				p_request = NULL ;
				return NULL ;
		...
		case E_light : {
			...
			rtn = Json_value ( inputData , "action" , p_request->m_action , sizeof ( p_request->m_action ) ) ;
			if ( D_success != rtn ) {
				free ( p_request ) ;
				p_request = NULL ;
				return NULL ;
			...
			p_request->i_action = 0 ;
			if ( strcmp ( p_request->m_action , "open" ) == 0 ) {
				p_request->i_action = 1 ;
			}
...
// 解析並回吐串列
Ble_in_list *Decode_and_rtn_list ( char *input_data ) {
	if ( ! input_data ) {
		return NULL ;
	}
	...
	return ( Ble_in_list * ) list_head ;
}

// 刪除串列
int Delete_ipc_list ( Ble_in_list **ipc_in_list ) {
...
Respon *Setup_response ( Ret_ssc *ret_ssc , Request *input_req , E_api input_e_api  ) {
	...
	switch ( input_e_api ) {
		case E_null :{
			...
			break ;
		case E_self_test : {
			in_request *req = ( in_request* ) input_req ;
			out_respon *rspn = ( out_respon* ) calloc ( 1 , sizeof(out_respon) ) ;
			if ( rspn == NULL ) {
				return NULL ;
			}
...
#define D_json_respon	"{\"api\":\"%s\",\"api_type\":\"%s\",\"ret_code\":\"%s\",\"ret_msg\":\"%s\",\"req_md5\":\"%s\"}"
...
Str Make_api_respon ( Respon *input_respon , E_api input_e_api ) {
	...
	switch ( input_e_api ) {
		case E_null : {
			...
			break ;
		case E_self_test : {
			out_respon *p_respon = ( out_respon * ) input_respon ;
			char json_respon [ strlen ( D_json_respon ) + strlen ( p_respon->m_api ) +
			                   strlen ( p_respon->m_api_type ) + strlen ( p_respon->m_ret_code ) +
			                   strlen ( p_respon->m_ret_msg ) + strlen ( p_respon->m_req_md5 ) + 1 ] ;
			memset ( json_respon , 0 , sizeof ( json_respon ) ) ;
			snprintf ( json_respon , sizeof ( json_respon ) , D_json_respon ,
					p_respon->m_api , p_respon->m_api_type , p_respon->m_ret_code ,
					p_respon->m_ret_msg , p_respon->m_req_md5 ) ;
